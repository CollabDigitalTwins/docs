...
When you create a new documentation section, add it to `sidebars.ts` to control its position and label:
...
const sidebars: SidebarsConfig = {
  baseSidebar: [
    ...
    {
      type: 'category',
      label: "Your New Section",  // ← Custom display label
      items: [
        {type: 'autogenerated', dirName: 'your-new-section'}  // ← Directory name
      ],
    },
  ]
};
...
- **Control over organization** - We decide the order sections appear in the sidebar
- **Custom labels** - Directory name can be `authorization` but display as "Authorization & Permissions"
- **Flexibility** - Easy to reorder, nest, or restructure documentation without renaming folders

The `autogenerated` type means Docusaurus will automatically include all files within that directory, maintaining alphabetical order within the section.
