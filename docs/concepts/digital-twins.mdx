...
Commercial digital twin platforms are powerful but proprietary: the data, the tools, and the infrastructure are controlled by a vendor. CDT takes the opposite approach. Everything is built on open standards and open-source software, which means:
...
CDT exposes the digital twin through two specialized viewers:
...
- **Point clouds** — potree-core streaming for large LiDAR and photogrammetry datasets, drawn in the BIM viewer alongside the models

Both viewers draw from the same underlying project data and share a coordinate system, so a building uploaded in the BIM viewer appears in its correct geographic position in the map viewer automatically.
