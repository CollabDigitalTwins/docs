...
CDT exposes data through two specialized viewers — Map and BIM — each optimized for a different data type and scale. They are not separate applications; they share the same underlying project data, coordinate system, and storage layer, and can be used in combination within a single session.
...
**Data types:** IFC, IDS, BCF, DXF, glTF/GLB, FBX, OBJ, LAS/LAZ/E57  
...
#### Point clouds
**Library:** [potree-core](https://github.com/tentone/potree-core) (Three.js-based)  
**Data types:** LAS, LAZ, E57  
**Scale:** Building → site → urban area

Reality capture datasets that would be impractical to load in a GIS tool are another kind of content the BIM viewer draws. Tile-based streaming and LOD keep billions of points interactive, and because a cloud is an object in the BIM scene it shares that scene's camera, clipping planes and measurement tools — which is what makes as-built against design a single comparison rather than two windows.

Uploads are converted server-side into a Potree octree; see the [point cloud guide](../guides/point-clouds.md).
