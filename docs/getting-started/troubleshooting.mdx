...
Most often the geocoder or tile server URL is missing. Check that `GEOCODE_EARTH_API_KEY` and `MARTIN_SERVER_URL` are set in `.env`. The geocoder is optional for local dev — set the key to any non-empty string to silence the warning.
...
`MINIO_BUCKET_URL` does not match how the browser actually reaches MinIO. In production this should be a public URL behind TLS — not an internal Docker hostname.
...
### "WebGL context lost" in the BIM viewer
...
`AUTH_URL` and `MINIO_BUCKET_URL` must match the public hostnames the browser uses. A mismatch causes the browser to request assets from URLs the proxy does not route.
