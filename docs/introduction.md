...
- **Two integrated viewers** — a MapLibre-powered web map and an IFC BIM viewer (That Open Engine), which draws models and point clouds in one scene — sharing a single coordinate system so a building uploaded in 3D appears in the right place on the map.
