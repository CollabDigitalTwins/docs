...
CDT's domain types are defined in `@collabdt/core/core/types/dbTypes.ts` and shared across hooks, API adapters, and components. Every entity maps directly to a Prisma model on the backend.
...
See [IFC Infrastructure Types](../concepts/ifc-infrastructure-types.mdx) for the full enum reference.
...
| `data` | `string` | Data source URL. Fetched verbatim, no prefix is added |
| `dataFormat` | `SensorDataFormat` | `Csv` (header-less `time,value`) or `Json` (auto-detects OGC SensorThings: STA Datastream, `dataArray`, or a single reading) |
| `updateFrequency` | `number` | Polling interval in milliseconds. The chart floors this at 1000ms (1s) |
...
| File | Role |
|------|------|
| `@collabdt/core/core/types/dbTypes.ts` | All domain interfaces and enums |
| `@collabdt/core/core/types/datasetTypes.ts` | Dataset and tile layer types |
| `@collabdt/core/core/types/global.ts` | Shared utility types (`Position`, etc.) |

## Related

- [IFC Infrastructure Types](../concepts/ifc-infrastructure-types.mdx): full IFC enum reference
- [State Management](./state-management.mdx): providers that hold these types in memory
