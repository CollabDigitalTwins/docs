...
Every domain in `@collabdt/core/store/` follows the same structure.
...
1. Create `@collabdt/core/store/<Domain>/reducer.ts` — define state type, action payload map, and reducer.
2. Create `@collabdt/core/store/<Domain>/context.tsx` — define the context and provider.
3. Register the provider in `CombineProviders.tsx`.
4. Export from `@collabdt/core/store/index.ts`.

## Related

- [State Management](./state-management.mdx) — full provider list, CombineProviders, and key files
